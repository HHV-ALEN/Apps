...
$ordenVenta = trim($_GET['OrdenVenta']  ?? '');
...
if ($ordenCompra !== '') {
    $oc  = $conn->real_escape_string($ordenCompra);
    $where .= " AND OrdenCompra LIKE '%$oc%'";
}
if ($ordenVenta !== '') {
    $oc  = $conn->real_escape_string($ordenVenta);
    $where .= " AND OrdenVenta LIKE '%$oc%'";
}
if ($numArticulo !== '') {
...
if ($codItem !== '') {
    $ci = $conn->real_escape_string($codItem);
    $where .= " AND CodItem LIKE '%$ci%'";
}


/* 2️⃣  Consulta de TOTAL de filas (para saber cuántas páginas hay) */
$sqlTotal = "SELECT COUNT(*) AS total FROM compras_cargacompras $where";
...
/* Consulta final */
$sql = "
  SELECT Id, OrdenCompra, OrdenVenta, Cliente, NumArticulo, Descripcion,
         Cantidad_Abierta, Precio, FechaContabilizacion, Pais
  FROM   compras_cargacompras
  $where
...
<body>
    ...
    <div class="container my-5">
        ...
        <ul class="nav nav-pills mb-4 justify-content-center">
            ...
            $paises = [
                ...
                'GDL' => ['nombre' => 'Guadalajara', 'bandera' => 'gdl', 'tipo' => 'logo', 'archivo' => 'Guadalajara.svg'],
            ];
                    ...
                    <div class="row g-3">
                        <div class="col-md-3">
                            ...
                            <input type="text" class="form-control"
                                name="OrdenCompra"
                                value="<?= htmlspecialchars($ordenCompra) ?>"
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="text" class="form-control"
                                name="OrdenVenta"
                                value="<?= htmlspecialchars($ordenVenta) ?>"
        ...
        <div class="card border-0 shadow-lg text-center">
            <div class="card-header bg-gradient-primary text-white py-3">
                <div class="d-flex justify-content-between align-items-center">
                    <h5 class="mb-0  text-dark">
                        <i class="bi bi-cart-check me-2 text-dark"></i>Compras – <?= htmlspecialchars($paisSel) ?>
                    </h5>
            ...
            foreach ($Registros as $registro) {
                if ($registro['Pais'] === 'MX' || $registro['Pais'] === 'GDL') {
                    $mostrarOV = true;
                    break;
                }
            }
                        ...
                        <tbody>
                            <?php foreach ($Registros as $index => $r): ?>
                                <tr class="position-relative">
                                    ...
                                    <td>
                                        <span class="badge bg-outline-primary bg-opacity-10 text-primary">
                                            <?= htmlspecialchars($r['OrdenCompra']) ?>
                                        </span>
                                    </td>

                                    <?php
                                    if ($r['Pais'] == 'MX' || $r['Pais'] == 'GDL') {
                                    ...
                                    <td class="pe-4 text-end">
                                        <a href="detallesCompraPestañas.php?id=<?= $r['Id'] ?>" class="btn btn-sm btn-outline-primary rounded-pill px-3">
    ...
    <script>
        document.getElementById('btnExportar').addEventListener('click', () => {
            ...
            window.location.href = 'exportarCompras.php?pais=' + encodeURIComponent(pais);
        });
    </script>
